// MessagesPlaceholder.tsx
// ─────────────────────────────────────────────────────────────────────────────
// Splash for the Messages tab. Empty inbox + 3-bullet preview.

import React, { useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion, useReducedMotion } from 'framer-motion';
import { MessageCircle, Megaphone, GraduationCap, BellRing } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center text-center px-6 pt-10 pb-6 min-h-full">
      <motion.div
        ...
        className="w-24 h-24 rounded-full bg-duo-orange-light inline-flex items-center justify-center shadow-[0_4px_0_0_#E08600] mb-5"
      >
        <MessageCircle className="w-12 h-12 text-duo-orange" strokeWidth={2.5} />
      </motion.div>

      <h1 className="text-2xl font-black text-slate-800 leading-tight mb-1">
        {t('parentApp.messages.headline')}
      </h1>
      <p className="text-sm font-extrabold text-slate-400 leading-relaxed mb-2">
        {t('parentApp.messages.empty')}
      </p>
      <p className="text-sm font-semibold text-slate-500 leading-relaxed max-w-[28ch] mb-6">
        {t('parentApp.messages.value')}
      </p>

      <div className="w-full space-y-2 mb-8">
        <FeatureBullet icon={Megaphone} text={t('parentApp.messages.feature1')} />
        <FeatureBullet icon={GraduationCap} text={t('parentApp.messages.feature2')} />
        <FeatureBullet icon={BellRing} text={t('parentApp.messages.feature3')} />
      </div>
...
const FeatureBullet: React.FC<FeatureBulletProps> = ({ icon: Icon, text }) => (
  <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/85 border border-white">
    <div className="w-8 h-8 rounded-full bg-duo-orange-light inline-flex items-center justify-center shrink-0">
      <Icon className="w-4 h-4 text-duo-orange" strokeWidth={2.5} />
    </div>
