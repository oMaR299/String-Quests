...
import { GradeUnitHeatmap } from './GradeUnitHeatmap';
import { TeacherEffectivenessTable } from './TeacherEffectivenessTable';
...
const SUBJECT_NAMES: Record<
  string,
  { ar: string; en: string; emoji: string; gradient: string }
> = {
...
const CAMPUS_MAP: Record<string, { ar: string; en: string }> = {
  elementary: { ar: 'المبنى الابتدائي', en: 'Elementary Campus' },
  middle: { ar: 'المبنى المتوسط', en: 'Middle Campus' },
  high: { ar: 'المبنى الثانوي', en: 'High Campus' },
};
...
function getCampus(grade: StudentProfile['grade']): string {
  if (grade === 'KG' || (typeof grade === 'number' && grade <= 5)) return 'elementary';
  if (typeof grade === 'number' && grade <= 9) return 'middle';
  return 'high';
}
...
interface TopicManagerDashboardProps {
  locale?: 'ar' | 'en';
...
export function TopicManagerDashboard({
  locale: initialLocale = 'ar',
  defaultSubject = 'math',
  onBack,
  className = '',
}: TopicManagerDashboardProps) {
  /* ── State ───────────────────────────────────────── */
  ...
  const subjectMeta = SUBJECT_NAMES[selectedSubject] || SUBJECT_NAMES.math;
  const subjectKey = selectedSubject as Exclude<Subject, 'all'>;

  /* ── Filtered students ───────────────────────────── */
  const filteredStudents = useMemo(() => {
    let list = MOCK_SCHOOL_DATA.filter(
      (s) => (s.subjectDetails[subjectKey]?.accuracy ?? 0) > 0,
    );
    if (campusFilter !== 'all') {
      list = list.filter((s) => getCampus(s.grade) === campusFilter);
    }
    return list;
  ...
  const summary = useMemo(() => {
    ...
    const avgAccuracy = Math.round(
      students.reduce((s, st) => s + (st.subjectDetails[subjectKey]?.accuracy ?? 0), 0) / total,
    );

    // engagement = % students with accuracy > 60
    const engaged = students.filter(
      (s) => (s.subjectDetails[subjectKey]?.accuracy ?? 0) > 60,
    ).length;
    ...
    for (const u of units) {
      const key = `${selectedSubject}-${u}`;
      const uStudents = students.filter((s) => s.lessonDetails[key]);
      if (uStudents.length === 0) continue;
      const avg = Math.round(
        uStudents.reduce((s, st) => s + (st.lessonDetails[key]?.accuracy ?? 0), 0) /
          uStudents.length,
      );
      if (avg < hardestUnitAcc) {
        hardestUnitAcc = avg;
        hardestUnit = u;
      }
    }
    ...
    // best campus
    const campuses = ['elementary', 'middle', 'high'];
    let bestCampus = campuses[0];
    let bestCampusAcc = 0;
    for (const c of campuses) {
      const cs = students.filter((s) => getCampus(s.grade) === c);
      if (cs.length === 0) continue;
      const avg = Math.round(
        cs.reduce((s, st) => s + (st.subjectDetails[subjectKey]?.accuracy ?? 0), 0) / cs.length,
      );
      if (avg > bestCampusAcc) {
        bestCampusAcc = avg;
        bestCampus = c;
      }
    }

    // sparkline: weekly mock
    const sparklineData = Array.from({ length: 8 }, (_, i) =>
      Math.max(50, avgAccuracy - 15 + Math.floor(Math.random() * 30) + i * 2),
    );
  ...
  }, [filteredStudents, subjectKey, selectedSubject, units]);
  ...
  const gradeLeaderboard = useMemo(() => {
    ...
    for (const s of filteredStudents) {
      ...
      // track top student per grade
      const curTopAcc = gradeMap.get(g)
        ? filteredStudents.find((st) => st.name === gradeMap.get(g)!.topStudent)?.subjectDetails[subjectKey]?.accuracy ?? 0
        : 0;
      if ((s.subjectDetails[subjectKey]?.accuracy ?? 0) > curTopAcc) {
        entry.topStudent = s.name;
      }

      gradeMap.set(g, entry);
    }

    const list = Array.from(gradeMap.values()).map((e) => ({
      ...e,
      avgXp: Math.round(e.avgXp / e.count),
      avgAccuracy: Math.round(e.avgAccuracy / e.count),
      trend: e.avgAccuracy / e.count > 78 ? ('up' as const) : e.avgAccuracy / e.count < 70 ? ('down' as const) : ('stable' as const),
    }));

    list.sort((a, b) =>
      gradeSortDir === 'desc'
        ? b[gradeSortKey] - a[gradeSortKey]
        : a[gradeSortKey] - b[gradeSortKey],
    );

    return list;
  }, [filteredStudents, subjectKey, gradeSortKey, gradeSortDir]);

  /* ── Unit deep dive stats ────────────────────────── */
  const unitStats = useMemo(() => {
    return units.map((u) => {
      ...
      const avgAcc =
        attempted.length > 0
          ? Math.round(
              attempted.reduce((s, st) => s + (st.lessonDetails[key]?.accuracy ?? 0), 0) /
                attempted.length,
            )
          : 0;
      const topStudent = attempted.sort(
        (a, b) => (b.lessonDetails[key]?.accuracy ?? 0) - (a.lessonDetails[key]?.accuracy ?? 0),
      )[0];
      return {
        unit: u,
        avgAccuracy: avgAcc,
        completion: total > 0 ? Math.round((attempted.length / total) * 100) : 0,
        studentCount: attempted.length,
        topStudent: topStudent?.name || '-',
      };
    });
  }, [units, selectedSubject, filteredStudents]);

  /* ── Top 10 students ─────────────────────────────── */
  const top10Students = useMemo(() => {
    return [...filteredStudents]
      .sort((a, b) => (b.subjectXp[subjectKey] || 0) - (a.subjectXp[subjectKey] || 0))
      .slice(0, 10);
  ...
  const trendData = useMemo(() => {
    ...
    return grades.map((g, gi) => {
      ...
      return {
        grade: g,
        color: colors[gi],
        points: weeks.map((_, wi) =>
          Math.max(40, Math.min(100, base + Math.floor(Math.random() * 15) - 5 + wi * 2)),
        ),
      };
    });
  }, []);
  ...
  const unitModalData = useMemo(() => {
    ...
    const key = `${selectedSubject}-${selectedUnitForPopup}`;
    const attempted = filteredStudents.filter((s) => s.lessonDetails[key]);
    const avgAcc =
      attempted.length > 0
        ? Math.round(
    ...
    const avgXp =
      attempted.length > 0
        ? Math.round(
            attempted.reduce((s, st) => s + (st.lessonDetails[key]?.xp ?? 0), 0) /
              attempted.length,
          )
        : 0;
    ...
    for (const s of attempted) {
      ...
      gradeGroups.get(g)!.push(s.lessonDetails[key]?.accuracy ?? 0);
    }
    ...
    const top5 = [...attempted]
      .sort((a, b) => (b.lessonDetails[key]?.accuracy ?? 0) - (a.lessonDetails[key]?.accuracy ?? 0))
      .slice(0, 5);
  ...
  }, [selectedUnitForPopup, selectedSubject, filteredStudents, locale]);

  /* ── Handlers ────────────────────────────────────── */
  const toggleGradeSort = (key: 'accuracy' | 'xp') => {
    if (gradeSortKey === key) {
      setGradeSortDir((d) => (d === 'desc' ? 'asc' : 'desc'));
    } else {
      setGradeSortKey(key);
      setGradeSortDir('desc');
    }
  };

  const openGradeUnitPopup = useCallback((_grade: number, _unit: string) => {
    // could open a detailed view — here we use unit popup
  }, []);
  ...
  return (
    <div
      dir={isRTL ? 'rtl' : 'ltr'}
      className={`min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 font-[Cairo] ${className}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <motion.header
          ...
          className="bg-white/80 backdrop-blur-sm border border-white/50 shadow-sm rounded-2xl p-5"
        >
          <div className="flex flex-wrap items-center gap-4">
            {/* Subject icon */}
            <div
              className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${subjectMeta.gradient} flex items-center justify-center text-2xl shadow-lg`}
            >
              {subjectMeta.emoji}
            </div>
            ...
            <div className="flex-1 min-w-[200px]">
              <h1 className="text-xl sm:text-2xl font-extrabold text-slate-800 leading-tight">
                {t('لوحة مدير المادة:', 'Topic Manager:')} {t(subjectMeta.ar, subjectMeta.en)}
              </h1>
              <p className="text-xs text-slate-400 mt-0.5">
                {t(
                  `${filteredStudents.length} طالب مسجل • ${units.length} وحدات`,
                  `${filteredStudents.length} enrolled students • ${units.length} units`,
                )}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative">
                <button
                  onClick={() => { setSubjectDropdownOpen(!subjectDropdownOpen); setCampusDropdownOpen(false); }}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                >
                ...
                <AnimatePresence>
                  {subjectDropdownOpen && (
                    <motion.div
                      ...
                      className="absolute top-full mt-1 z-50 bg-white border border-slate-200 rounded-xl shadow-xl p-1.5 min-w-[200px] max-h-72 overflow-y-auto"
                    >
                      {Object.entries(SUBJECT_NAMES).map(([key, val]) => (
                        <button
                          key={key}
              ...
              <div className="relative">
                <button
                  onClick={() => { setCampusDropdownOpen(!campusDropdownOpen); setSubjectDropdownOpen(false); }}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
                >
                  ...
                  <span>
                    {campusFilter === 'all'
                      ...
                      : t(CAMPUS_MAP[campusFilter].ar, CAMPUS_MAP[campusFilter].en)}
                  </span>
                  <ChevronDown className="w-4 h-4 text-slate-400" />
                </button>
                <AnimatePresence>
                  {campusDropdownOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, scale: 0.95, y: -4 }}
                      transition={{ duration: 0.15 }}
                      className="absolute top-full mt-1 z-50 bg-white border border-slate-200 rounded-xl shadow-xl p-1.5 min-w-[180px]"
                    >
                      <button
                        onClick={() => { setCampusFilter('all'); setCampusDropdownOpen(false); }}
                        className={`w-full px-3 py-2 rounded-lg text-sm font-medium text-start transition-colors ${
                          campusFilter === 'all' ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50'
                        }`}
                      >
                        {t('جميع المباني', 'All Campuses')}
                      </button>
                      {Object.entries(CAMPUS_MAP).map(([key, val]) => (
                        <button
                          key={key}
                          onClick={() => { setCampusFilter(key); setCampusDropdownOpen(false); }}
                          className={`w-full px-3 py-2 rounded-lg text-sm font-medium text-start transition-colors ${
                            campusFilter === key ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50'
                          }`}
                        >
                          {t(val.ar, val.en)}
                        </button>
              ...
              <div className="flex items-center bg-slate-100 rounded-xl p-0.5">
                {([
                  { key: 'week' as Timeframe, ar: 'هذا الأسبوع', en: 'This Week' },
                  { key: 'month' as Timeframe, ar: 'هذا الشهر', en: 'This Month' },
                  { key: 'all' as Timeframe, ar: 'الكل', en: 'All' },
                ] as const).map((tf) => (
                  <button
                    key={tf.key}
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
              >
              ...
              {onBack && (
                <button
                  onClick={onBack}
                  className="flex items-center gap-1 px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors"
                >
        ...
        <SectionBlock
          icon={BarChart3}
          title={t('الملخص التنفيذي', 'Executive Summary')}
          subtitle={t('نظرة عامة على أداء المادة', 'Subject performance overview')}
          idx={1}
          accentGradient={subjectMeta.gradient}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Card 1: Total Students */}
            <motion.div
              whileHover={{ y: -2 }}
              className="bg-white/80 backdrop-blur-sm border border-white/50 shadow-sm rounded-2xl p-5 flex items-start gap-4"
            >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-semibold text-slate-400 mt-0.5">
                  {t('متوسط الدقة', 'Average Accuracy')}
                </p>
                <div className="mt-2">
                  <Sparkline
            ...
            <motion.div
              whileHover={{ y: -2 }}
              className="bg-white/80 backdrop-blur-sm border border-red-100/60 shadow-sm rounded-2xl p-5 flex items-start gap-4"
            >
            ...
            <motion.div
              whileHover={{ y: -2 }}
              className="bg-white/80 backdrop-blur-sm border border-emerald-100/60 shadow-sm rounded-2xl p-5 flex items-start gap-4"
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-lg font-extrabold text-slate-800">
                  {t(
                    CAMPUS_MAP[summary.bestCampus]?.ar || summary.bestCampus,
                    CAMPUS_MAP[summary.bestCampus]?.en || summary.bestCampus,
                  )}
                </p>
                <p className="text-xs font-semibold text-emerald-500 mt-0.5">
                  {t('أفضل مبنى', 'Best Campus')} — {summary.bestCampusAcc}%
                </p>
                <div className="mt-2 flex items-center gap-1">
                  <div className="h-2 rounded-full bg-emerald-100 flex-1">
                    <div
                      className="h-2 rounded-full bg-emerald-500 transition-all"
                      style={{ width: `${summary.bestCampusAcc}%` }}
                    />
                  </div>
        ...
        <SectionBlock
          icon={Layers}
          title={t('خريطة الأداء الحرارية', 'Grade Performance Heatmap')}
          subtitle={t('مقارنة الوحدات عبر الصفوف', 'Compare units across grades')}
          idx={2}
          accentGradient={subjectMeta.gradient}
        >
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm overflow-x-auto">
            <GradeUnitHeatmap
              ...
              locale={locale}
              onCellClick={openGradeUnitPopup}
            />
          </div>
        </SectionBlock>
        ...
        <SectionBlock
          icon={Trophy}
          title={t('ترتيب الصفوف', 'Grade Leaderboard')}
          subtitle={t('مقارنة أداء الصفوف في المادة', 'Compare grade performance in this subject')}
          idx={3}
          accentGradient={subjectMeta.gradient}
        >
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-[48px_1fr_1fr_100px_1fr_80px_48px_1fr] gap-2 px-5 py-3 bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-500 items-center">
              ...
              <button
                onClick={() => toggleGradeSort('accuracy')}
                className="flex items-center gap-1 hover:text-slate-700 transition-colors"
              >
                {t('متوسط الدقة', 'Avg Accuracy')}
                <ArrowUpDown className="w-3 h-3" />
              </button>
            ...
            {gradeLeaderboard.map((row, i) => (
              <motion.div
                ...
                className={`grid grid-cols-[48px_1fr_1fr_100px_1fr_80px_48px_1fr] gap-2 px-5 py-3 items-center border-b border-slate-50 hover:bg-slate-50/50 transition-colors ${
                  i < 3 ? 'bg-gradient-to-r from-amber-50/40 to-transparent' : ''
                }`}
              >
                {/* Rank */}
                ...
                {/* Grade badge */}
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold w-fit bg-gradient-to-r ${subjectMeta.gradient} text-white`}
                >
                ...
                <span className="text-xs font-semibold text-slate-500">
                  {t(CAMPUS_MAP[row.campus]?.ar || row.campus, CAMPUS_MAP[row.campus]?.en || row.campus)}
                </span>

                {/* Avg XP */}
                <span className="text-sm font-bold text-slate-700">
                  {row.avgXp.toLocaleString()}
                </span>
        ...
        <SectionBlock
          icon={GraduationCap}
          title={t('فعالية المعلمين', 'Teacher Effectiveness')}
          subtitle={t('أداء المعلمين عبر الوحدات', 'Teacher performance across units')}
          idx={4}
          accentGradient={subjectMeta.gradient}
        >
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm overflow-x-auto">
            <TeacherEffectivenessTable
              subject={selectedSubject}
              students={filteredStudents}
              locale={locale}
              onStudentClick={(st: StudentProfile) => setSelectedStudent(st)}
            />
          </div>
        </SectionBlock>
        ...
        <SectionBlock
          icon={BookOpen}
          title={t('تفاصيل الوحدات', 'Unit Deep Dive')}
          subtitle={t('استكشف أداء كل وحدة', 'Explore each unit\'s performance')}
          idx={5}
          accentGradient={subjectMeta.gradient}
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {unitStats.map((u, i) => (
              <motion.button
                ...
                onClick={() => setSelectedUnitForPopup(u.unit)}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-slate-800 truncate">
                      {t(UNIT_LABELS_AR[u.unit] || u.unit, u.unit)}
                    </p>
                    <p className="text-[10px] font-semibold text-slate-400">
                      {u.studentCount} {t('طالب', 'students')}
                    </p>
                  </div>
                  <span
                    className="text-lg font-extrabold"
                    style={{ color: accuracyColor(u.avgAccuracy) }}
                  >
                    {u.avgAccuracy}%
                  </span>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="h-1.5 rounded-full bg-white/60">
                    <div
                      className="h-1.5 rounded-full transition-all bg-slate-400/50"
                ...
                <p className="mt-3 text-[10px] font-semibold text-slate-500">
                  {t('أفضل طالب:', 'Top student:')} {u.topStudent}
                </p>
              </motion.button>
            ))}
          </div>
        ...
        <SectionBlock
          icon={BarChart3}
          title={t('الرسوم البيانية', 'Analytics Charts')}
          subtitle={t('اتجاهات وتوزيعات الأداء', 'Performance trends and distributions')}
          idx={6}
          accentGradient={subjectMeta.gradient}
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-700 mb-4">
                {t('اتجاه الدقة متعدد الصفوف', 'Multi-Grade Accuracy Trend')}
              </h3>
              <svg
                viewBox="0 0 400 220"
                className="w-full"
              ...
              >
                <defs>
                  {trendData.map((line) => (
                    <linearGradient
                      key={line.grade}
                      id={`trend-area-${trendChartId}-${line.grade}`}
                      x1="0"
                      y1="0"
                      x2="0"
                      y2="1"
                ...
                {/* Grid lines */}
                {[40, 60, 80, 100].map((v) => {
                  const y = 180 - ((v - 30) / 80) * 170;
                  return (
                    <g key={v}>
                      <line x1={40} x2={380} y1={y} y2={y} stroke="#e2e8f0" strokeWidth={0.5} />
                ...
                {/* X-axis labels */}
                {trendData[0].points.map((_, wi) => {
                  const x = 40 + (wi / 7) * 340;
                  return (
                    <text key={wi} x={x} y={198} textAnchor="middle" fontSize={9} fill="#94a3b8">
                      {t(`أ${wi + 1}`, `W${wi + 1}`)}
                    </text>
                  );
                })}
                ...
                {trendData.map((line) => {
                  ...
                  return (
                    <g key={line.grade}>
                      ...
                      <circle
                        cx={40 + 340}
                        cy={180 - ((line.points[7] - 30) / 80) * 170}
                        r={3}
                        fill={line.color}
                      />
                    </g>
                ...
                {trendData.map((line) => (
                  <div key={line.grade} className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-semibold text-slate-500">
                      {t(`الصف ${line.grade}`, `Grade ${line.grade}`)}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-700 mb-4">
                {t('الدقة مقابل نقاط الخبرة', 'Accuracy vs XP')}
              </h3>
              <AccuracyVsXpScatter
                students={filteredStudents.map((s) => ({
                  id: s.id,
                  name: s.name,
                  xp: s.subjectXp[subjectKey] || 0,
                  accuracy: s.subjectDetails[subjectKey]?.accuracy || 0,
                  league: s.league,
        ...
        <SectionBlock
          icon={Eye}
          title={t('رؤى التدخل', 'Intervention Insights')}
          subtitle={t('توصيات لتحسين الأداء', 'Recommendations for improvement')}
          idx={7}
          accentGradient={subjectMeta.gradient}
        >
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
        ...
        <SectionBlock
          icon={Crown}
          title={t('أفضل 10 طلاب', 'Top 10 Students')}
          subtitle={t('ترتيب الطلاب حسب نقاط الخبرة', 'Student ranking by XP in this subject')}
          idx={8}
          accentGradient={subjectMeta.gradient}
        >
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            {top10Students.map((st, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={`w-full flex items-center gap-3 px-5 py-3.5 border-b border-slate-50 hover:bg-slate-50/60 transition-colors text-start ${
                    i < 3 ? 'bg-gradient-to-r from-amber-50/50 to-transparent' : ''
                  }`}
                >
                  {/* Rank */}
                  <span className="w-8 text-center text-sm font-extrabold text-slate-700 shrink-0">
                    {i < 3 ? MEDAL_EMOJIS[i] : i + 1}
                  </span>

                  {/* Avatar circle */}
                  <div
                    className={`w-9 h-9 rounded-full ${st.avatar || 'bg-blue-100'} flex items-center justify-center text-sm font-bold text-slate-600 shrink-0`}
                  >
                    {st.name.charAt(0)}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[10px] font-semibold text-slate-400">
                        {t(`الصف ${st.grade}`, `Grade ${st.grade}`)} / {st.section}
                      </span>
                      <span className="text-[10px] font-semibold text-slate-300">|</span>
                      <span className="text-[10px] font-semibold text-slate-400">
                        {t(CAMPUS_MAP[getCampus(st.grade)]?.ar, CAMPUS_MAP[getCampus(st.grade)]?.en)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-center shrink-0">
                    <p className="text-sm font-extrabold text-slate-700">{xp.toLocaleString()}</p>
                    <p className="text-[9px] font-semibold text-slate-400">XP</p>
                  </div>
                    ...
                    <div className="flex items-center gap-1">
                      <div className="h-2 rounded-full bg-slate-100 flex-1">
                        <div
                          ...
                          style={{
                            width: `${acc}%`,
                            backgroundColor: accuracyColor(acc),
                          }}
                        />
                      </div>
                      <span className="text-[10px] font-bold" style={{ color: accuracyColor(acc) }}>
                        {acc}%
                      </span>
                    </div>
                  </div>
                  ...
                  {/* League badge */}
                  <span
                    className={`px-2 py-0.5 rounded-md text-[10px] font-bold shrink-0 ${leagueStyle.bg} ${leagueStyle.text}`}
                  >
                    {t(leagueStyle.label, st.league)}
                  </span>
                </motion.button>
              );
            })}
      ...
      <AnimatePresence>
        {selectedUnitForPopup && unitModalData && (
          ...
          >
            ...
            >
              {/* Gradient header */}
              <div className="bg-gradient-to-r from-sky-500 to-blue-600 rounded-t-2xl px-6 py-5 relative">
                <button
                  onClick={() => setSelectedUnitForPopup(null)}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-extrabold text-white">
                      {t(
                        UNIT_LABELS_AR[selectedUnitForPopup] || selectedUnitForPopup,
                        selectedUnitForPopup,
                      )}
                    </h2>
                    <span
                      className={`inline-block mt-1 px-2.5 py-0.5 rounded-lg text-xs font-bold bg-white/20 text-white`}
                    >
                      {subjectMeta.emoji} {t(subjectMeta.ar, subjectMeta.en)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-4 text-center">
                    <p className="text-2xl font-extrabold text-emerald-600">{unitModalData.avgAcc}%</p>
                  ...
                  <div className="rounded-xl bg-sky-50 border border-sky-100 p-4 text-center">
                    <p className="text-2xl font-extrabold text-sky-600">
                      {unitModalData.avgXp.toLocaleString()}
                    </p>
                    <p className="text-[10px] font-semibold text-sky-500 mt-0.5">
                      {t('متوسط XP', 'Avg XP')}
                    </p>
                  </div>
                  <div className="rounded-xl bg-violet-50 border border-violet-100 p-4 text-center">
                    <p className="text-2xl font-extrabold text-violet-600">{unitModalData.studentCount}</p>
                ...
                {unitModalData.gradeComparison.length > 0 && (
                  <div>
                    <h3 className="text-sm font-bold text-slate-700 mb-3">
                      {t('مقارنة الصفوف', 'Grade Comparison')}
                    </h3>
                    <VerticalBarChart
                      data={unitModalData.gradeComparison}
                      maxValue={100}
                      showValues
                    />
                  </div>
                    ...
                    <div className="space-y-2">
                      {unitModalData.top5.map((st, i) => {
                        ...
                        return (
                          <div
                            key={st.id}
                            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border border-slate-100 ${
                              i < 3 ? 'bg-amber-50/40' : 'bg-slate-50/40'
                            }`}
                          >
                            <span className="text-sm font-extrabold text-slate-600 w-6 text-center">
                              {i < 3 ? MEDAL_EMOJIS[i] : i + 1}
                            </span>
                            <div
                              className={`w-8 h-8 rounded-full ${st.avatar || 'bg-blue-100'} flex items-center justify-center text-xs font-bold text-slate-600`}
                            >
                              {st.name.charAt(0)}
                            </div>
                            <div className="flex-1 min-w-0">
                            ...
                            <span
                              ...
                              style={{ color: accuracyColor(stAcc) }}
                            >
                              {stAcc}%
                            </span>
                          </div>
      ...
      {(subjectDropdownOpen || campusDropdownOpen) && (
        <div
          className="fixed inset-0 z-40"
          onClick={() => { setSubjectDropdownOpen(false); setCampusDropdownOpen(false); }}
        />
      )}
    </div>
  );
...
export default TopicManagerDashboard;
