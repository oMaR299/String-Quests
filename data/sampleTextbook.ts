/**
 * Textbook Data Module
 *
 * Provides the Textbook > Unit > Lesson > Page > KnowledgeComponent (KC)
 * type hierarchy used by the Skill Map and related components.
 *
 * Data is sourced from the real curriculum (data/curricula/mathCurriculum.ts)
 * via the getCurriculumAsTextbook adapter for grades 1-12 (+ 12B Business Math).
 *
 * Legacy Grade 3 sample data is kept below for reference but is no longer the
 * primary source. The shared lookup maps (KC_MAP, PAGE_MAP, LESSON_MAP,
 * UNIT_MAP) contain ALL data (legacy + curriculum) so every helper function
 * and consumer component works uniformly.
 */

import { BloomLevel } from './skillTaxonomy';
import { getCurriculumAsTextbook } from './curricula';
...
// ─── Flat Lookup Maps ──────────────────────────────────────────────────────────
// These are populated first with the legacy Grade 3 seed data, then expanded by
// the curriculum adapter for all 13 textbooks (grades 1-12 + 12B).
...
// ─── Legacy Grade 3 Seed Data ─────────────────────────────────────────────────
// Kept for backward-compatibility with any code that references kc-3m-* IDs.
// The curriculum adapter for Grade 3 will use its own kc-math-g3-* IDs.

const LEGACY_KCS: KnowledgeComponent[] = [
...
const LEGACY_PAGES: TextbookPage[] = [
...
// Register legacy data into the maps
for (const kc of LEGACY_KCS) { KC_MAP[kc.id] = kc; }
for (const page of LEGACY_PAGES) { PAGE_MAP[page.id] = page; }
for (const lesson of LEGACY_LESSONS) { LESSON_MAP[lesson.id] = lesson; }
for (const unit of LEGACY_UNITS) { UNIT_MAP[unit.id] = unit; }

// ─── Curriculum-sourced Textbooks (grades 1-12 + 12B) ─────────────────────────
// The adapter converts CurriculumFramework data to the Textbook type system and
// registers all generated objects into the shared maps above.

const _sharedMaps = { KC_MAP, PAGE_MAP, LESSON_MAP, UNIT_MAP };

export const AVAILABLE_TEXTBOOKS: Textbook[] = [
  getCurriculumAsTextbook(0, _sharedMaps),   // Grade 1
  getCurriculumAsTextbook(1, _sharedMaps),   // Grade 2
  getCurriculumAsTextbook(2, _sharedMaps),   // Grade 3
  getCurriculumAsTextbook(3, _sharedMaps),   // Grade 4
  getCurriculumAsTextbook(4, _sharedMaps),   // Grade 5
  getCurriculumAsTextbook(5, _sharedMaps),   // Grade 6
  getCurriculumAsTextbook(6, _sharedMaps),   // Grade 7
  getCurriculumAsTextbook(7, _sharedMaps),   // Grade 8
  getCurriculumAsTextbook(8, _sharedMaps),   // Grade 9
  getCurriculumAsTextbook(9, _sharedMaps),   // Grade 10
  getCurriculumAsTextbook(10, _sharedMaps),  // Grade 11
  getCurriculumAsTextbook(11, _sharedMaps),  // Grade 12
  getCurriculumAsTextbook(12, _sharedMaps),  // Grade 12B (Business Math)
];

/** @deprecated Use AVAILABLE_TEXTBOOKS[2] (Grade 3) or getTextbookById() instead */
export const TEXTBOOK_DATA: Textbook = AVAILABLE_TEXTBOOKS[2]; // Grade 3
...
// ─── Helper Functions ──────────────────────────────────────────────────────────
// All helpers now read from the shared maps so they cover both legacy and
// curriculum-sourced data.

/** Get all KCs as an array */
export function getAllKCs(): KnowledgeComponent[] {
  return Object.values(KC_MAP);
}
